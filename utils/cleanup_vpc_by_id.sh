...
do
	aws ec2 terminate-instances --instance-ids ${INSTANCES[$i]} > /dev/null 2>&1
...
do
	aws ec2 delete-security-group --group-id ${SECURITY_GROUPS[$i]} > /dev/null 2>&1
...
do
	aws ec2 delete-subnet --subnet-id ${SUBNETS[$i]} > /dev/null 2>&1
...
do
	aws ec2 delete-route-table --route-table-id ${ROUTE_TABLES[$i]} > /dev/null 2>&1
...
aws ec2 detach-internet-gateway --internet-gateway-id $INTERNET_GATEWAY_ID --vpc-id $1 > /dev/null 2>&1
...
aws ec2 delete-vpc --vpc-id $1 > /dev/null 2>&1
#echo VPC $1 DELETED
